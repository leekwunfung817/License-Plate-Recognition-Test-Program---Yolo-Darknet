...
#pragma comment(lib, "darknet/pthreads/lib/x64/pthreadVC2.lib")
...
// OCR_RESULT *pResult 表示返回找到的位置
//source_name 原始图片名称, 用于保存临时文件, 空表示不保存
image plate_detect_cpos(char detect_type, network *net, image source_image, float thresh,
	float hier_thresh,int letter_box, char *app_path, char **names, OCR_RESULT *pResult, char *source_name, int is_save_train, int is_save_image, float area_left, float area_top, float area_right, float area_bottom)
{
	...
	if (source_image.w == 0 || source_image.h == 0 || source_image.c == 0 || source_image.data == 0) return im_plate;

	CRect rect_area;//设定的识别区域

	
	rect_area = CRect::CRect((int)(area_left*source_image.w), (int)(area_top*source_image.h),
		(int)(area_right*source_image.w), (int)(area_bottom*source_image.h));
	if (rect_area.Width() <= 1)
	{
		rect_area = CRect::CRect(0, 0,
	...
	do
	{
		...
		//if (detect_type == 0)
		//{
		//SaveStrToTxt(_T("e:\\save_txt.txt"), TRUE, _T("0"));
		//	save_image(sized, "e:\\save0.jpg");
		//}
		//else if (detect_type == 1)
		//{
		//SaveStrToTxt(_T("e:\\save_txt.txt"), TRUE, _T("1"));
		//	save_image(sized, "e:\\save1.jpg");
		//}
		//else if (detect_type == 2)
		//{
		//	save_image(sized, "e:\\save2.jpg");
		//SaveStrToTxt(_T("e:\\save_txt.txt"), TRUE, _T("2"));
		//}
			...
			if (class_id >= 0 && prob > 0.5)//可靠性超过0.7
			{
				...
				if (pResult && detect_type == 0)
				{
					...
					if(class_id == 0)
					{
						...
						//CRect plate_area;//设定的识别区域
						//plate_area = CRect::CRect((int)((float)x / source_image.w), (int)((float)y / source_image.h),
						//(int)((float)(x+width) / source_image.w), (int)((float)(y+height) / source_image.h)
						//	);

						
						POINT pt1 = { x, y };
						POINT pt2 = { x + width, y };
						POINT pt3 = { x, y + height };
						POINT pt4 = { x + width, y + height};
						if (rect_area.PtInRect(pt1)
							&& rect_area.PtInRect(pt2)
							&& rect_area.PtInRect(pt3)
							&& rect_area.PtInRect(pt4)
							)
						{
							if (im_plate.data)
							{
...
//	char ocr_coco_data[512] = { 0 };
	...
	//			char *name_list = option_find_str(options, "names", "data/names.list"); //在参数列表中是否找到目录
			...
			//识别车型
			plate_detect_cpos(2, &g_net_car_type, im_src, thresh, 0.5, 0, app_path, g_car_type_names, letter_result, source_picture_file, save_train, save_image, 0, 0, 0, 0);
		...
		//	OCR_RESULT plate_result;
			//识别铁牌的位置
			im_plate = plate_detect_cpos(0, &g_net_plate, im_src, thresh, 0.5, 0, app_path, 0, pResult, source_picture_file, save_train, save_image, area_left, area_top, area_right, area_bottom);
...
//			if (im_plate.data) save_image(im_plate, "plate");	//保存图片,调试用
			...
			//识别铁牌上的字符
			im_ocr = plate_detect_cpos(1, &g_net_letter, im_plate, thresh, 0.5, 0, app_path, g_letter_names, letter_result, source_picture_file, save_train, save_image, 0, 0, 0, 0);
			if (im_ocr.data)
			{
				free_image(im_ocr);
			}
