...
//pPath返回程序运行目录 , 目录带最后的斜线

void get_app_path(char *pPath, int len)
{
	WCHAR path[512];
	GetModuleFileName(NULL, path, 512);
	WCHAR *pLast;
	pLast = wcsrchr(path, L'\\');	//从右向左查找字符
	if (pLast)
	{
		path[pLast - path + 1] = L'\0';
	}

	WideCharToMultiByte(CP_ACP, 0, path, -1, pPath, len - 1 , NULL, NULL);
	return;
}
